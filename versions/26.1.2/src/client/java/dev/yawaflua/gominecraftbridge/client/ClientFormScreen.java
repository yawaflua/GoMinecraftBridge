...
final class ClientFormScreen extends Screen {
		...
		for (ClientScreenField field : this.spec.fields()) {
			if ("select".equals(field.type())) {
				...
				if ("password".equals(field.type())) {
					edit.addFormatter((value, offset) -> FormattedCharSequence.forward("*".repeat(value.length()), Style.EMPTY));
				}
	...
	public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
		...
		for (int index = this.hitboxes.size() - 1; index >= 0; index--) {
			...
			if (event.x() >= position.x() && event.x() < position.x() + element.width()
					&& event.y() >= position.y() && event.y() < position.y() + element.height()) {
	...
	public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
		...
		if (this.spec.body() != null && !this.spec.body().isBlank()) {
			graphics.centeredText(this.font, this.spec.body(), this.width / 2, 38, 0xbfbfbf);
		}
		int index = 0;
		for (ClientScreenField field : this.spec.fields()) {
			int y = 60 + index * 40 - this.scroll;
			if (y >= 48 && y < this.height - 30) {
				graphics.text(this.font, field.label(), this.width / 2 - WIDTH / 2, y, 0xd0d0d0, false);
			}
			index++;
		}
	}

	private void renderCustomElements(GuiGraphicsExtractor graphics) {
		for (ClientScreenElement element : this.spec.elements()) {
			...
			if (element.type().equals("rectangle")) {
				...
				graphics.text(this.font, element.text() == null ? "" : element.text(), position.x(), position.y(), color, element.shadow());
			}
		}
	}
